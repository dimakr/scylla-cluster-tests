...
class AwsSctRunner(SctRunner):
    ...
    def _create_instance(self,
        ...
        if os.getenv("SCT_CLUSTER_BACKEND") == "docker":
            ...
            LOGGER.info("Getting unused disks...")
            disks = self._get_unused_disks(remoter)
            LOGGER.info("The following unused disks are detected: %s", disks)
            if disks:
                mount_point = "/mnt/docker-persistent-storage"
                self._setup_disks(remoter, disks, mount_point)
                LOGGER.info("Disks are configured.")
                self._set_docker_data_root(remoter, mount_point)
                LOGGER.info("Persistent storage for Docker is configured.")
            else:
